...
**Do not report the task as complete until Step 10 (Lint Gate) passes.**
...
- `@TestCaseMetadata`: `description`, `priority` (0=highest → 5=lowest), `timeout`, `requirement`, `tags`.
...
- **AI marker (mandatory)**: Every `@TestCaseMetadata` produced by this prompt must declare it is AI-generated, in two ways:
  1. Ensure the final non-empty line of `description` is exactly: `This test is generated by the lisa_test_writer prompt.` (no leading/trailing whitespace; no trailing blank line).
  2. Add `tags=["ai-generated"]` to the `@TestCaseMetadata`.
...
class SriovValidation(TestSuite):
    @TestCaseMetadata(
        description=(
            "Verify at least one SR-IOV VF device exists via lspci.\n"
...
## Step 9: AI-Generated Labeling (Mandatory)

Both the commit and PR **must** carry the `[AI Generated]` prefix for traceability.
Never omit it; if the user proposes a title without it, supply the corrected version.

- **Commit subject**: `[AI Generated] <area>: <summary>` (`<area>` = `@TestSuiteMetadata(area=...)`, <72 chars).
- **PR title**: `[AI Generated] <same subject>`.
- **PR description** should include:
  ...
  ```
- **After the user confirms**, give the exact commands:
  ```bash
  git add lisa/microsoft/testsuites/<feature_area>/<test_name>.py
  git commit -m "[AI Generated] <feature_area>: <summary>"
  ```

---

## Step 10: Lint Gate (MANDATORY before declaring done)
