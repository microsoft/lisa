    ...
    def _verify_lis_preinstall_disk_size(
        ...
        try:
            if LisDriver.can_install:
                ...
                boot_part_required_space = (
                    ramdisk_required_space + boot_partition_buffer_space
                )
                ...
                if test_type == "negative":
                    ...
                    fallocate.create_file(
                        single_partition_file_size, "/lib/modules/file.out"
                    )
                if boot_partition != root_partition:
                    ...
                    fallocate.create_file(
                        single_partition_file_size_boot, "/boot/file.out"
                    )
                ...
                raise SkippedException(
                    f"current distro {node.os.name} doesn't support install LIS"
                )
        finally:
            self._clean_up_files(node)
