...
def _get_smb_version(node: Node) -> str:
    if _is_module_enabled(node, "CONFIG_CIFS_SMB311"):
        version = "3.1.1"
    else:
        version = "3.0"
    return version
...
def _is_module_enabled(node: Node, config_name: str) -> bool:
    ...
    config = f"{config_name}=y|{config_name}=m"
    result = node.execute(
...
class Xfstesting(TestSuite):
    ...
    def xfstesting_azure_file_share_validation(
        ...
        if not _is_module_enabled(node, "CONFIG_CIFS"):
