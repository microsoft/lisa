...
@TestSuiteMetadata(
    area="vm_extension",
    category="functional",
    description="BVT for the Azure Performance Diagnostics VM Extension",
)
class AzurePerformanceDiagnostics(TestSuite):
    ...
    @TestCaseMetadata(
        description="""
        Installs and deletes the Azure Performance Diagnostics VM Extension on the VM.
        """,
        priority=3,
    ...
    def _is_supported_linux_distro(self, node: Node) -> bool:
        # TODO - REMOVE
        logger = get_logger("init", "test")
        logger.info(">>>>> " + str(node.os.information.version.major))
        logger.info(">>>>> " + str(node.os.information.version.minor))
        ...
        # ONOTE TODO - is there a better way than 3 things in this if statement?
        if (
    ...
    def _is_unsupported_version(self, node: Node) -> bool:
        # These are specific Linux distro versions that are not supported by Azure Performance Diagnostics, even though the major version is generally supported
        ...
        if unsupported_versions_list != None:
