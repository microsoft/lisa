...
class azureFirewallTests(TestSuite):  
    ...
    def setup_azureFirewall(self, environment: Environment, log: Logger) -> None:
        ...
        result = firewallNode.execute("sudo tdnf install -y azure-cli", sudo=True) #Done
        ...
        log.info("Result for installing azure-cli:", result) #Done
...
def createRouteTable(environment: Environment, log: Logger):
    ...
    client.features[NetworkInterface].create_route_table(
        ...
        subnet_mask= ipv4_to_lpm(all_available_nics[3].ip_addr),
        em_first_hop= all_available_nics[2].ip_addr+"/32",
        next_hop_type= "VirtualAppliance",
        dest_hop= all_available_nics[0].ip_addr
    )

    # server.features[NetworkInterface].create_route_table(
    #     nic_name= all_available_nics[2].name,
    #     route_name= "serverToFirewall",
    #     subnet_mask= ipv4_to_lpm(all_available_nics[7].ip_addr),
    #     em_first_hop= all_available_nics[7].ip_addr+"/32",
    #     next_hop_type= "VirtualAppliance",
    #     dest_hop= all_available_nics[0].ip_addr
    # )
...
def firewallInit(firewallNode, log):
    ...
    #Setup Azure CLI and Login to Azure
    setupAzureCLI(firewallNode, log)

    #Download the necessary files to setup Azure Firewall 
    for fileName in filestoDownload:
        downloadFilesFromBlob(fileName, fileName, lisaStorageAccountName, lisaContainerName, firewallNode, log)
    #Download App Version 
    downloadFilesFromBlob(bootstrapFileName, "bootstrap.tar", gsaStorageAccountName, gsaContainerName, firewallNode, log)
    # generatemdsMetadata(firewallNode,log)

    performWorkArounds(firewallNode,log)
    runBootstrap(firewallNode,log)

def runBootstrap(node, log):
    ...
    # escaped_json = json_str.replace('"', '\\"')
    command = f"/tmp/bootstrap/drop/vmss/bootstrap.sh '{json_str}'"
    result = node.execute(f"bash -x {command}", sudo=True)
    log.info("Successfully executed bootstrap.sh", result)
