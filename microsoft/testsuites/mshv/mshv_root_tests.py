...
class MshvHostTestSuite(TestSuite):
    ...
    def verify_mshv_crash(
        ...
        log.debug(f"mshv_chk_bin: {chkbinpath}")
        log.debug(f"mshv_chk_loader: {chkloaderpath}")
        ...
        if not isinstance(node.os, CBLMariner):
            raise SkippedException(
                f"Testcase only support Mariner-MSHV. Found: {node.os}"
            )
        ...
        copy_tool.copy(
            src=PurePath(chk_loader_extract_dir) / "boot" / "efi" / "lxhvloader.dll",
        ...
        kdump_util = KdumpUtils(crashdump_timeout=800, kdump_cmd=mshv_crash_command)
        kdump_util._kdump_test(
            node=node,
            log_path=log_path,
            log=log,
            is_auto=False,
