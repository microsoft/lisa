...
from microsoft.testsuites.kata_conformance.kata_conformance_tests_tool import KataConformanceTests
...
def validate_variable(variables,key,log,nullable=False):
    ...
    if((not nullable and value is None) or (not nullable and type(value)==str and value.strip()=="")):
...
@TestSuiteMetadata(
    ...
    category="community",
    description="""
    This test suite is for executing the conformance test with kata container with AKS cluster.
    """,
)
class KataConformanceTestSuite(TestSuite):
    ...
    def kata_conformance_tests(
        ...
        log.info("Variables "+json.dumps(variables))
        subscription_id = validate_variable(variables,"subscription_id",log,nullable=True)
        client_id = validate_variable(variables,"service_principal_client_id",log,nullable=True)
        client_secret = validate_variable(variables,"service_principal_key",log,nullable=True)
        tenant_id = validate_variable(variables,"service_principal_tenant_id",log,nullable=True)
        ...
        headers = {
            "AKSHTTPCustomFeatures":validate_variable(variables,"aks_http_custom_features",log),
            "OSImageSubscriptionID":validate_variable(variables,"os_image_subscription_id",log),
            "OSImageResourceGroup":validate_variable(variables,"os_image_resource_group",log),
        ...
        aks = AKSInfra(log=log,subscription_id=subscription_id,client_id=client_id,client_secret=client_secret,tenant_id=tenant_id)
        aks.create_aks_infra(kubernetes_version,worker_vm_size,node_count,azure_region,headers)
