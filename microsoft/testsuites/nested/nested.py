...
class Nested(TestSuite):
    ...
    def _check_partition_capacity(
        ...
        unused_partition_size_in_gb = (partition.total_size - partition.used) / (
            1024 * 1024
        )
    ...
    def _get_partition_for_nested_image(self, node: RemoteNode) -> str:
        home_partition = node.tools[Df].get_partition_with_mountpoint("/home")
        if home_partition and self._check_partition_capacity(home_partition):
            return home_partition.mountpoint

        mnt_partition = node.tools[Df].get_partition_with_mountpoint("/mnt")
    ...
    def _connect_nested_vm(
        ...
        # start nested vm
        host.tools[Qemu].create_nested_vm(
