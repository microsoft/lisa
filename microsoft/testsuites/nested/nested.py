...
from lisa.features import Nested
from lisa.tools import Cat, Echo, Lsmod, Lsblk, Mkfs
from lisa.operating_system import Suse, CoreOs, CentOs, Oracle, Redhat, Ubuntu, Debian


def install_sshpass(self, node: Node, log: Logger) -> None:
    ...
    if cmd_result.exit_code != 0:
        ...
        if exit_code != 0:
            ...
            node.execute("./configure --prefix=/usr/ && make && make install", sudo = True)
            node.execute("cd ..")
        else:
...
def remove_raid(self, disks: List[str], node: Node, log: Logger) -> None:
    ...
    for result in cmd_result:
        if "md" in result:
            ...
            for disk in disks:
                ...
                node.tools[Mkfs].mkfs(f"/dev/{disk}", "mkfs.ext4")
    

def create_raid0(self, disks: List[str], device_name: str, node: Node, log: Logger) -> None:
    count = 0
    for disk in disks:
        log.debug(f"Partition disk /dev/{disk}")
        # TODO: CHECK THIS LINE AND LINE 84
        node.execute(
            f"(echo d; echo n; echo p; echo 1; echo; echo; echo t; echo fd; echo w;) | fdisk /dev/{disk}", sudo=True
        )
    ...
    time.sleep(1)
    log.debug(f"Run cmd: yes | mdadm --create {device_name} --level 0 --raid-devices {count} {raid_devices}")
    cmd_result = node.execute(
        f"yes | mdadm --create {device_name} --level 0 --raid-devices {count} {raid_devices}", sudo=True
    )
    cmd_result.assert_exit_code(
        message=f"Unable to create raid {device_name}."
    )

    log.debug(f"Raid {device_name} create successfully.")

 
def install_kvm_dependendcies(self, node: Node, log: Logger) -> None:
    ...
    if isinstance(node.os, CentOs) or isinstance(node.os, Redhat) or isinstance(node.os, Oracle):
        ...
        node.execute(
            "[ -f /usr/libexec/qemu-kvm ] && ln -s /usr/libexec/qemu-kvm /sbin/qemu-system-x86_64", shell=True, sudo=True
        )
    
    cmd_result = node.execute(
        "which qemu-system-x86_64", sudo=True
    )
    if cmd_result.exit_code != 0:
    ...
    if exit_code == 0:
        ...
        node.execute(
            "wget https://github.com/q3aql/aria2-static-builds/releases/download/v1.35.0/aria2-1.35.0-linux-gnu-64bit-build1.tar.bz2", sudo=True
        )
        node.execute(
            "tar -xf aria2-1.35.0-linux-gnu-64bit-build1.tar.bz2", sudo=True
        )
        node.execute(
            "cd aria2-1.35.0-linux-gnu-64bit-build1/", sudo=True
        )
        node.execute(
            "make install", sudo=True
        )
        node.execute(
            "cd ..", sudo=True
        )
...
def download_image_files(self, destination_image_name: str, source_image_url: str, node: Node, log: Logger) -> None:
    node.execute(
        "[ ! -d /mnt/resource ] && mkdir /mnt/resource", sudo = True
    )
    log.debug(f"Downloading {source_image_url}")

    node.execute(
        f"rm -f /mnt/resource/{destination_image_name}", sudo = True
    )
    cmd_result = node.execute(
        f"aria2c -d /mnt/resource -o {destination_image_name} -x 10 {source_image_url}", sudo = True
    )
    if cmd_result.exit_code != 0:
...
def prepare_nested_vm(self, disks: List[str], user: str, password: str, port: str, nested_cpu_num: int, nested_mem_mb: int, image_name: str, host_fwd_port: int, node: Node, log: Logger) -> None:
    ...
    reboot_nested_vm(user, password, port, image_name, node, log)


def run_fio(self, node: Node, nested_user_password: str, host_fwd_port: int) -> None:
    remote_copy()

    remote_exec('localhost', 'root', nested_user_password, '/root/StartFioTest.sh', host_fwd_port, node, log)


def stop_nested_vm(self, node: Node, log: Logger) -> None:
...
def add_sles_network_utilities_repo(self, distro_version: str, node: Node, log: Logger) -> int:
    # if distro_name == "sles" or "sle_hpc":
    if isinstance(node.os, Suse):
        if distro_version.startswith("11"):
            ...
            log.debug(f"Unsupported SLES version {distro_version} for add_sles_network_utilities_repo")

        check_install_lock_sles(node, log)

        node.execute(
            f"zypper addrepo {repo_url}", shell=True, sudo=True
        )
        node.execute(
            f"zypper --no-gpg-checks refresh", shell=True, sudo=True
        )
        return 0
    else:
...
def check_install_lock_sles(self, node: Node, log: Logger) -> None:
    pid = node.execute(
        "pidof zypper", sudo = True
    ).stdout
    if pid:
...
def update_repos(self, node: Node, log: Logger) -> int:
    # if distro_name == "oracle" or "rhel" or "centos":
    if isinstance(node.os, Oracle) or isinstance(node.os, Redhat) or isinstance(node.os, CentOs):
...
def install_package(self, package_list: List[str], node: Node, log: Logger) -> int:
...
def check_package(self, package_list: List[str], node: Node, log: Logger) -> int:
    for package in package_list:
        # if distro_name == "oracle" or "rhel" or "centos":
        if isinstance(node.os, Oracle) or isinstance(node.os, Redhat) or isinstance(node.os, CentOs):
...
def install_epel(self, node: Node, log: Logger) -> int:
    distro_version = node.os.information.version
    # if distro_name == "oracle" or "rhel" or "centos":
    if isinstance(node.os, Oracle) or isinstance(node.os, Redhat) or isinstance(node.os, CentOs):
        cmd_result = node.execute(
            "yum -y install epel-release", sudo = True
            )
        ...
        if cmd_result.exit_code != 0:
            if "6." in distro_version:
                ...
                return 1
            node.execute(
                f"sudo rpm -ivh {epel_rpm_url}", sudo = True
            )
...
def install_net_tools(self, node: Node) -> None:
    distro_version = node.os.information.version
    # if ((distro_name == "sles") and (distro_version.startswith("15"))) or (distro_name == "sle_hpc"):
    if ("sles" in node.os.name and distro_version.startswith("15")) or "sle_hpc" in node.os.name:
        node.execute(
    ...
    if isinstance(node.os, Ubuntu):
        node.execute(
            "apt_get_install 'net-tools' > /dev/null 2>&1", sudo = True
        )


def start_nested_vm(self, user: str, password: str, port: str, cmd: str, node: Node, log: Logger) -> None:
    ...
    while exit_status != 0 and retry_times > 0:
        ...
        if retry_times == 0:
            ...
            time.sleep(10)
            log.debug(f"Try to connect to the nested VM, left retry times: {retry_times}")
...
def enable_root(self, user: str, password: str, port: str, image_name:str, node: Node, log: Logger) -> None:
    ...
    remote_exec('localhost', user, password, f"chmod a+x /home/{user}/*.sh", port, node, log)
    remote_exec('localhost', user, password, f"echo {password} | sudo -S /home/{user}/enable_root.sh -password {password}", port, node, log)

    log.debug(f"Root enabled for VM: {image_name}")


def reboot_nested_vm(self, user: str, password: str, port: str, node: Node, log: Logger) -> None:
    if user == "" or password == "" or port == "":
        return #CHANGE THIS
    
    log.debug("Reboot the nested VM")
    remote_exec('localhost', user, password, f"echo {password} | sudo -S reboot", port, node, log)
    log.debug("Wait for the nested VM to boot up ...")
    ...
    while exit_status != 0 and retry_times > 0:
        if retry_times == 0:
            ...
            time.sleep(10)
            log.debug(f"Try to connect to the nested VM, left retry times: {retry_times}")
            exit_status = remote_exec('localhost', user, password, 'hostname', port, node, log)
...
def remote_copy() -> None:
    123

def remote_exec(self, host: str, user: str, passwd: str, cmd: str, port: str, node: Node, log: Logger) -> None:
    ...
    exit_code = node.execute(
        f"sshpass -p {passwd} ssh -t -o StrictHostKeyChecking=no -p {port} {user}@{host} {cmd} 2>&1", sudo = True
    ).exit_code
    ...
    log.debug(f"Executing cmd: {cmd} on host {host} through port {port} by user {user} succeeded.")
...
class nested(TestSuite): 
    ...
    def nested_kvm_storage_single_disk(self, variables: Dict[str, Any], node: Node, log: Logger) -> None:
        nested_image_url = variables.get("nested_image_url", self.nested_url)
        nested_user = variables.get("nested_user", self.nested_user)
        nested_password = variables.get("nested_user_password")

        self.nested_kvm_storage_test(nested_user, nested_password, nested_image_url, "No RAID", node, log)
    ...
    def nested_kvm_storage_test(
        ...
        assert_that(nested_user_password).describe_as(
        ...
        if raid_option == "RAID in L1":
            ...
            create_raid0(available_disks, md_volume)
            # if cmd_result.exit_code:   # THIS REQUIRES COMMAND_RESULT CONTAINS EXIT CODE
            #     return
            available_disks = "md0"

        for disk in available_disks:
            node.tools[Echo].write_to_file("0", f"/sys/block/{disk}/queue/rq_affinity", sudo = True)
        ...
        # Prepare nested kvm
        prepare_nested_vm(node, available_disks, nested_user, nested_user_password, self.host_fwd_port , self.nested_cpu_num, self.nested_mem_mb, image_name, self.host_fwd_port, log)
