...
class Kselftest(Tool):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._test_collections = test_collections.split(",")
    ...
    def _generate_run_tests(
        ...
        self._log.debug(f"test_collections: {test_collections}, skip_tests: {skip_tests}, list_file:{test_list_file}")
        ls = self.node.tools[Ls]
        if not ls.path_exists(test_list_file.as_posix()):
            raise LisaException("kselftest-list.txt is empty")

        self._log.debug(f"test_count: {len(test_collections)}, skip_count: {len(skip_tests)}")
