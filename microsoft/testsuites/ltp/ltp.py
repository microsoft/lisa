...
class Ltp(Tool):
    ...
    def command(self) -> str:
        return f"{self._command_path}runltp"

    @property
    def dependencies(self) -> List[Type[Tool]]:
        if self._prebuild_package:
            return []
        else:
            return [Make, Gcc, Git]
    ...
    def __init__(self, node: Node, prebuild_package: str, *args: Any, **kwargs: Any) -> None:
        super().__init__(node, args, kwargs)
        git_tag = kwargs.get("git_tag", "")
        self._git_tag = git_tag if git_tag else self.DEFAULT_LTP_TESTS_GIT_TAG

        self._prebuild_package = prebuild_package
        if self._prebuild_package:
    ...
    def _install(self) -> bool:
        ...
        if self._prebuild_package:
            remote_tar_path = self.node.get_pure_path(self._command_path) / os.path.basename(self._prebuild_package)
            ...
            self.node.shell.copy(PurePath(self._prebuild_package), remote_tar_path)
            self.node.tools[Tar].extract(str(remote_tar_path), str(ltp_installed_dir), sudo=True)
