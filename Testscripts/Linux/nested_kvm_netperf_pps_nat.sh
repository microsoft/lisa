...
Start_Nested_VM_Nat()
{
    ...
    Log_Msg "qemu-system-x86_64 -cpu ${CPU_MODEL} -smp $NestedCpuNum -m $NestedMemMB -hda /mnt/resource/$image_name -device $NestedNetDevice,netdev=net0,mac=$mac_addr1 -netdev user,id=net0,hostfwd=tcp::$host_fwd_port-:22 \
    ...
    cmd="qemu-system-x86_64 -cpu ${CPU_MODEL} -smp $NestedCpuNum -m $NestedMemMB -hda /mnt/resource/$image_name -device $NestedNetDevice,netdev=net0,mac=$mac_addr1 -netdev user,id=net0,hostfwd=tcp::$host_fwd_port-:22 \
