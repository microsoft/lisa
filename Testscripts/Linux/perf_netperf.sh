...
ssh ${server} "killall netserver"
ssh ${client} "killall netperf"
...
if [[ "$test_type" == "singlepps" ]]; then
    ...
    client_sar_command="sar -n DEV 1 ${test_duration} > netperf-client-sar-output.txt"
    ...
    while [ $current_job -le $max_jobs ]; do
        while [ $current_port -le $max_port ]; do
            client_command="netperf -H ${server} -p $current_port -t TCP_RR -n 32 -l ${test_duration} -D 1 -- -O 'THROUGHPUT, THROUGHPUT_UNITS, MIN_LATENCY, MAX_LATENCY, MEAN_LATENCY, REQUEST_SIZE, RESPONSE_SIZE, STDDEV_LATENCY' >> netperf-client-output.txt 2>&1"
            ssh ${client} $client_command &
