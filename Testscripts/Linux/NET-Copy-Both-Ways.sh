...
iptables -F
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "iptables -F"
...
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "dd if=${file_source} of=${tmp}/${output_file_2} bs=1M count=$((file_size/1024/1024))"
if [ 0 -ne $? ]; then
...
remote_md5sum_file_2=$(ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "md5sum ${tmp}/${output_file_2} |  cut -f 1 -d ' '")
...
"
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "echo '${remote_cmd}' > send_file.sh"
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "setsid bash send_file.sh >/dev/null 2>&1 < /dev/null &"
scp -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$tmp"/"$output_file_1" "$remote_user"@"$STATIC_IP2":"$tmp"/"$output_file_1"
if [ 0 -ne $? ]; then
...
remote_exit_status=$(ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "cat ${remote_exit_status_file_path}")
...
remote_md5sum_file_1=$(ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "md5sum ${tmp}/${output_file_1} | cut -f 1 -d ' '")
local_md5sum_file_2=$(md5sum ${tmp}/${output_file_2} | cut -f 1 -d ' ')

# delete files
rm -f "$tmp"/$output_file_1
rm -f "$tmp"/$output_file_2
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "rm -f ${tmp}/${output_file_1}"
ssh -i "$HOME"/.ssh/"$SSH_PRIVATE_KEY" -o StrictHostKeyChecking=no "$remote_user"@"$STATIC_IP2" "rm -f ${tmp}/${output_file_2}"
