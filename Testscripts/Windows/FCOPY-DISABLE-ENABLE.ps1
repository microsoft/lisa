...
function Main {
    ...
    $checkProcess = $daemonName = Run-LinuxCmd -username $VMUserName -password $VMPassword -ip $Ipv4 -port $VMPort  "systemctl is-active $daemonName" -runAsSudo
    if ($checkProcess -ne "active") {
        Write-LogErr "Warning: $daemonName was not automatically started by systemd. Will start it manually."
        $null = .\Tools\plink.exe -C -pw $VMPassword -P $VMPort $VMUserName@$Ipv4 " systemctl start $daemonName"
    }
