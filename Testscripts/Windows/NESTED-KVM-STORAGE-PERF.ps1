...
function Send-ResultToDatabase ($currentTestResult, $AllVMData, $currentTestData) {
	...
	for ( $QDepth = $startThread; $QDepth -le $maxThread; $QDepth *= 2 ) {
		if ($testResult -imatch $resultPass) {
			...
			$resultMap["L1GuestDistro"] = Get-Content "$LogDir\VM_properties.csv" | Select-String "OS type"| ForEach-Object{$_ -replace ",OS type,",""}
			$resultMap["L1GuestSize"] = $L1GuestSize
			$resultMap["L1GuestKernelVersion"] = Get-Content "$LogDir\VM_properties.csv" | Select-String "Kernel version"| ForEach-Object{$_ -replace ",Kernel version,",""}
