...
function Main {
    ...
    [int]$majorVersion = Run-LinuxCmd -username $guestUsername -password $VMPassword -ip $vm2ipv4 `
        -port $VMPort -command ". utils.sh && GetOSVersion && echo `$os_RELEASE"
    [int]$minorVersion = Run-LinuxCmd -username $guestUsername -password $VMPassword -ip $vm2ipv4 `
    ...
    $kernel = Run-LinuxCmd -username $guestUsername -password $VMPassword -ip $vm2ipv4 `
    ...
    Run-LinuxCmd -username $guestUsername -password $VMPassword -ip $vm2ipv4 -port $VMPort -command ". utils.sh; test_rsync_files" -ignoreLinuxExitCode:$true
    $state = Run-LinuxCmd -username $guestUsername -password $VMPassword -ip $vm2ipv4 -port $VMPort -command "cat state.txt" -ignoreLinuxExitCode:$true
    Write-LogInfo "State file on VM1 has the following content $state"
    if ($state -notMatch "Completed") {
