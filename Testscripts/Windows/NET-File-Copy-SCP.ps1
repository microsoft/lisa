...
function Connect-Disk {
    ...
    LogMsg "$driveName has been mounted to /mnt in the VM"
    return $True
}
...
function Initialize-File {
    ...
    if ($createfile -notlike "File *testfile-* is created") {
        LogErr "Could not create the sample test file in the working directory! $file_path_formatted"
        exit -1
    }
...
function Get-LocalMD5 {
    ...
    if (-not $?){
        ...
        LogMsg "MD5 checksum on Hyper-V: $localChksum"
    }
    return $localChksum
}
...
function Remove-Files {
    ...
    if (-not $?) {
        LogErr "Cannot remove the test file '${FilePathFormatted1}'!"
    }
    Remove-Item -Path \\$HvServer\$FilePathFormatted2 -Force
    if (-not $?) {
        LogErr "Cannot remove the test file '${FilePathFormatted2}'!"
    }
}
...
function Main {
    ...
    # Using pscp because remoteCopy function doesn't have the option of a specific location
    $Error.Clear()
    ...
    if (-not $job) {
        LogErr "Job to transfer the 2nd file failed to start!"
        return "FAIL"
    }
    ...
    if($jobInfo.State -ne "Running") {
        LogErr "job did not start or terminated immediately!"
        return "FAIL"
    }
    ...
    if ($Error.Count -eq 0) {
        LogMsg "File has been successfully copied to guest VM"
    } else {
        LogErr "An error occured while copying files!"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }

    LogMsg "The file copy process took $([System.Math]::Round($copyDuration1, 2)) minutes for first file `
    ...
    if (-not $sts) {
        LogErr "1st file is not present on the guest VM!"
        return "FAIL"
    } elseif ($sts -eq $filesize1) {
        "The 1st file copied matches the 10GB size."
    } else {
        LogErr "The 1st file copied doesn't match the 10GB size!"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }
    ...
    if (-not $sts[-1]) {
        LogErr "2nd file is not present on the guest VM!"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    } elseif ($sts[0] -eq $filesize2) {
        LogMsg "The 2nd file copied matches the 10GB size."
    } else {
        LogErr "The 2nd file copied doesn't match the 10GB size!"
        Remove-Files
        return "FAIL"
    }
    ...
    # first file
    $md5RemoteFile1 = RunLinuxCmd -username $guestUsername -password $VMPassword -ip $IPv4 `
        -port $VMPort -command "openssl md5 /mnt/$testfile1" -ignoreLinuxExitCode:$true
    if (-not $?) {
        LogErr "Unable to run openssl command on VM for first file!"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }
    ...
    LogMsg "MD5 checksum for 1st file on Guest VM: $remoteChecksum1"

    $md5IsMatching1 = [string]::Compare($remoteChecksum1, $localFile1MD5, $true)
    if ($md5IsMatching1 -ne 0) {
        LogErr "MD5 checksums are not matching for first file"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }
    LogMsg "MD5 checksums are matching for first file"

    # 2nd file
    $md5RemoteFile2 = RunLinuxCmd -username $guestUsername -password $VMPassword -ip $IPv4 `
        -port $VMPort -command "openssl md5 /mnt/$testfile2" -ignoreLinuxExitCode:$true
    if (-not $?) {
        LogErr "Unable to run openssl command on VM for second file"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }
    ...
    LogMsg "MD5 checksum for 2nd file on Guest VM: $remoteChecksum2"

    $md5IsMatching2 = [string]::Compare($remoteChecksum2, $localFile2MD5, $true)
    if ($md5IsMatching2 -ne 0) {
        LogErr "MD5 checksums are not matching for second file"
        Remove-Files $HvServer $filePathFormatted1 $filePathFormatted2
        return "FAIL"
    }
    LogMsg "MD5 checksums are matching for the second file"
