function GenerateConstantsFile ($filePath) {
...
function  ExecuteTest ($ip, $port) {
	...
	$cmd = "/home/$user/$($currentTestData.testScript) -logFolder /home/$user > /home/$user/TestExecutionConsole.log"
...
function UploadResultToDatabase ($xmlConfig, $logDir) {
...
function Main () {
	...
	if ($isDeployed) {
		try	{
			...
			GenerateConstantsFile -filePath $constantsFile
			RemoteCopy -uploadTo $hs1VIP -port $hs1vm1sshport -files ".\$constantsFile" -username $user -password $password -upload

			ExecuteTest -ip $hs1VIP -port $hs1vm1sshport

			# Download test logs
			$files = "/home/$user/state.txt, /home/$user/$($currentTestData.testScript).log, /home/$user/TestExecutionConsole.log"
			...
			if ($testResult -imatch "PASS")	{
				...
				if (!$uploadResults) {
					...
					UploadResultToDatabase -xmlConfig $xmlConfig -logDir $LogDir
				}
			}
		}
		catch
