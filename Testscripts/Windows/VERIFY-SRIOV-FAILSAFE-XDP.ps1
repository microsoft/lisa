...
function Ping-XDPDump {
    ...
    $currentStatus = Run-LinuxCmd -ip $VMData.PublicIP -port $VMData.SSHPort -username $user -password $password `
        -command "tail -1 ~/xdpdumpout$LogSuffix.txt | head -1" -runAsSudo
...
"@
        ...
        if ($currentState -imatch "TestCompleted") {
            ...
            $testResult = Ping-XDPDump $clientVMData $serverVMData.SecondInternalIP "eth1" "_SRIOV"
            ...
            $testResult = Ping-XDPDump $clientVMData $serverVMData.SecondInternalIP "eth1" "_Synthetic"
