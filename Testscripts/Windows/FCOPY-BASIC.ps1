    ...
    try {
        ...
        if (-not $gsi.Enabled) {
            ...
            if ((Get-VM -ComputerName $HvServer -Name $VMname).State -ne "Off") {
                Stop-VM -ComputerName $HvServer -Name $VMname -Force -Confirm:$false | Out-Null
            }
            ...
            Enable-VMIntegrationService -Name "Guest Service Interface" -VMname $VMname -ComputerName $HvServer | Out-Null
            Write-LogInfo "Starting VM:'${VMname}'"
            $Ipv4 = Start-VMandGetIP $VMname $HvServer $VMPort $VMUserName $VMPassword
            # The ip adress may be changed after restarting the VM
            $AllVMData.PublicIP = $Ipv4
        }
        ...
        $null = Run-LinuxCmd -username $VMUserName -password $VMPassword -ip $Ipv4 `
            -port $VMPort -command "[ -d /tmp ] || mkdir /tmp"
        ...
        $null = Run-LinuxCmd -username $VMUserName -password $VMPassword -ip $Ipv4 `
            -port $VMPort -command "rm -f /tmp/testfile-*"
        ...
        Copy-VMFile -VMname $VMname -ComputerName $HvServer -SourcePath $filePath -DestinationPath "/tmp/" `
            -FileSource host -ErrorAction SilentlyContinue | Out-Null
        if ($Error.Count -eq 0) {
        ...
        $sts = Check-FileInLinuxGuest -VMPassword $VMPassword -VMPort $VMPort -VMUserName $VMUserName `
            -Ipv4 $Ipv4 -fileName "/tmp/$testfile" -checkSize $true
        if (-not $sts) {
        ...
        Remove-Item -Path \\$HvServer\$file_path_formatted -Force | Out-Null
        if ($? -ne "True") {
