...
#   count parameter and check that the VM reboots successfully without
#   event 18602 generated
...
function Main {
    ...
    while ($count -gt 0) {
        ...
        if ((Get-VMEvent -VMName $VMName -HvServer $hvServer -StartTime $testStartTime `
                -EventID 18602 -RetryCount 2 -RetryInterval 1)) {
    ...
    Start-VM -Name $VMName -ComputerName $hvServer -Confirm:$false
    Write-LogInfo "VM rebooted $bootcount times successfully and not trigger a critical event 18602 on the host"
    return "PASS"
}
