...
Function Invoke-IngestKustoFromTSQL([string]$SQLString) {
	try {
		...
		if ($kustoClusterURI -and $kustoDatabase -and $kustoDataDLLPath) {
			...
			$insertTSQLPattern = "(?imn:^\s*INSERT\s+INTO\s+(?<tablename>\w+)\s*\(\s*(?<columns>(\w+\s*,\s*)+\w+)\s*\)\s*VALUES\s*(\(\s*(?<values>(\'[^\']*\',)*\s*\'[^\']*\')\s*\)\s*,*\s*)+$)"
			...
			if ($allMatches.Matches) {
				foreach ($matchGroup in $allMatches.Matches.Groups) {
					...
					elseif ($matchGroup.Name -eq "values") {
						$valueArr = @($matchGroup.Captures.Value)
					}
				}
				if ($tableName -and $columns -and $valueArr) {
					...
					foreach ($rowValue in $valueArr) {
						...
						$ingestInlineCmd += "`n $($ingestValueArray -Join ',')"
					}
			...
			Write-LogErr "Missing 'KustoClusterURI' or 'KustoDatabase' or 'KustoDataDLLPath' from Secrets file, hence skip Kusto ingestion"
		}
	}
	catch {
