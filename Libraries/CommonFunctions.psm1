...
function Mount-Disk{
 ...
 <#
    ...
    $sts = Send-CommandToVM -vmPassword $vmPassword -vmPort $vmPort -ipv4 $ipv4  "mkfs.ext3 ${driveName}1"
    if (-not $sts) {
...
function Copy-Check-File{
    ...
    if ($Error.Count -eq 0) {
        ...
        elseif ($sts[0] -ne $filesize) {
            LogErr "ERROR: The copied file doesn't match the $filesize size." 
            return $False
        }
        ...
        if ($sts)
        {
            if (test-path $localStateFile)
            {
                ...
                if ($null -ne $contents)
                {
                        ...
                        {
                             LogMsg  "Info : State file contains TestAborted failed. "
                             break
...
function Create-Controller{
    ...
    {
        LogErr "Error: Bad SCSI controller ID: $controllerID"
        return $False
    }
    ...
    {
       LogMsg "Info : SCSI controller already exists"
    }
    else
    {
        $error.Clear()
        ...
        if ($error.Count -gt 0)
        {
           LogErr "    Error: Add-VMScsiController failed to add 'SCSI Controller $ControllerID'"
            $error[0].Exception
            return $False
        }
        LogMsg "Info : Controller successfully added"
    }
    return $True
}
...
function Stop-FcopyDaemon{
    ...
    if ($sts[-1] -eq $True ){
        ...
        if (-not $?) {
            LogErr "ERROR: Unable to kill hypervfcopy daemon"
            return $False
        }
    }
    return $true
...
function Get-ParentVHD {
    ...
    $VmInfo = Get-VM -Name $vmName -ComputerName $hvServer
    if (-not $VmInfo) {
       LogErr "Error: Unable to collect VM settings for ${vmName}"
       return $False
    }
...
function Convert-ToMemSize {
    ...
    if ($memString.EndsWith("MB")) {
        ...
        if (-not $osInfo) {
            LogMsg "Error: Unable to retrieve Win32_OperatingSystem object for server ${hvServer}"
            return $False
        }
