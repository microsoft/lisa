...
if [ "$SKIP_PYTHON" = false ]; then
    ...
    if command_exists python3 && version_ge "$CURRENT_VERSION" "3.8"; then
        ...
        # Check if current version meets LISA requirements (3.9+)
        if ! version_ge "$CURRENT_VERSION" "3.9"; then
            echo "  [WARN] Python $CURRENT_VERSION is too old. LISA requires Python 3.9+."
...
# Upgrade pip (skip for Ubuntu 24.04+ as it will be done in venv)
if [ "$OS" = "ubuntu" ] && version_ge "$OS_VERSION" "24.04"; then
...
# Determine if we should use virtual environment (for Ubuntu 24.04+ to avoid PEP 668)
USE_VENV=false
if [ "$OS" = "ubuntu" ] && version_ge "$OS_VERSION" "24.04"; then
    USE_VENV=true
...
if [ "$USE_VENV" = true ]; then
    ...
    if ! python3 -m pip install --editable '.[azure,libvirt]' --config-settings editable_mode=compat; then
        echo "  [ERROR] LISA installation failed"
        echo ""
        echo "  Common causes:"
