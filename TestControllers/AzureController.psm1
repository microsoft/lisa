...
Class AzureController : TestController
{
	...
	[void] PrepareTestImage() {
		# If 'TestLocation' and 'OsVHD' are both present, try to copy/move OsVHD to the target storage account (which is selected by TestLocation)
		# If 'TestLocation' is not present, skip copy/move OsVHD, until LISA has selected TestLocation (Region) and resources are ready to be deployed
		if ($this.OsVHD -and $this.TestLocation) {
			$null = Move-OsVHDToStorageAccount -OriginalOsVHD $this.OsVHD -TargetStorageAccount $this.GlobalConfig.Global.Azure.Subscription.ARMStorageAccount
		}
	}
