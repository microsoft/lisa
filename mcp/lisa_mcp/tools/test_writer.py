...
from lisa_mcp.tools._repo import (
    ...
    load_context_file,
    load_docs_for_tool,
    load_test_writer_prompt,
)


def register_test_writer_tools(mcp: FastMCP) -> None:
    @mcp.tool()
...
'''
    ...
    def lisa_list_test_requirements(test_name: str) -> str:
        ...
        Args:
            test_name: Exact test method name (e.g. "smoke_test", "verify_sriov_failover")
        """
    ...
    def lisa_write_test(
        ...
        found_tools = _search_repo_symbols(
            repo_root, "lisa/tools", search_query
        )
        found_features = _search_repo_symbols(
            repo_root, "lisa/features", search_query
        )
        found_suites = _search_repo_symbols(
            repo_root, "lisa/microsoft/testsuites", search_query,
        )
        ...
        if found_suites:
            ...
            if repo_root:
                ...
                query_kws = set(
                    w.lower()
                    for w in re.split(r"\W+", search_query)
                    if len(w) > 2
                )
                # Only flag methods matching specific (non-common) keywords
                specific_kws = query_kws - {
                    "verify", "test", "check", "module", "kernel",
                    "functional", "basic", "config", "storage",
                    "network", "core", "should", "whether",
                }
        ...
        dirty_keywords = [
            "kernel", "grub", "driver", "reboot", "module", "modprobe",
            "sysctl", "network config", "insmod", "rmmod",
        ]
        needs_mark_dirty = any(
            kw in what_to_validate.lower() for kw in dirty_keywords
        )
        ...
        meta_lines.append(
            f"**Required Features:** {', '.join(found_features[:5]) if found_features else 'none detected'}"
        )
        meta_lines.append(
            f"**Required Tools:** {', '.join(found_tools[:5]) if found_tools else 'none detected'}"
        )
        sections.append("\n".join(meta_lines) + "\n")
        ...
        sections.append(
            ...
            "Once confirmed, use `lisa_scaffold_test_suite` or `lisa_scaffold_test_case` "
    ...
    for suite_name in suite_files[:5]:
        ...
        for i, line in enumerate(lines):
            ...
            method_match = re.match(
                r"def\s+((?:verify|test)_\w+)\s*\(", stripped
            )
            if method_match:
...
def _search_repo_symbols(
    ...
    keywords = [
        ...
        if len(w) > 2 and w.lower() not in {
            "the", "and", "for", "test", "with", "that", "are", "this",
            "from", "have", "has", "was", "were",
        }
    ]
    ...
    for py_file in search_dir.rglob("*.py"):
        ...
        try:
            ...
            for match in re.finditer(
                r"def\s+((?:verify|test)_\w+)\s*\(", content
            ):
