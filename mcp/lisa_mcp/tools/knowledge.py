...
def register_knowledge_tools(mcp: MCPServer) -> None:  # noqa: C901
                    ...
                    while end < len(lines):
                        ...
                        if (
                            ...
                            and (
                                ...
                                or line.startswith("\t" * (indent // 4 or 1))
                            )
    ...
    def lisa_find_examples(query: str, max_results: int = 5) -> str:
        ...
        for testsuites_dir in testsuites_dirs:
            ...
            for py_file in testsuites_dir.rglob("*.py"):
                ...
                if score > 0:
                    class_match = re.search(r"class\s+(\w+)\(TestSuite\)", content)
                    ...
                    methods = re.findall(r"def\s+((?:verify_|test_)\w+)\s*\(", content)
                    if methods:
        ...
        for _score, path, summary in top:
    ...
    def lisa_list_features() -> str:
        ...
        for py_file in sorted(features_dir.glob("*.py")):
            ...
            for m in re.finditer(r"class\s+(\w+)\([^)]*Feature[^)]*\):", content):
                name = m.group(1)
                after = content[m.end() :]
        ...
        return (
            ...
            "Declare in test: "
            "`simple_requirement(supported_features=[FeatureName])`\n\n"
            + "\n".join(features)
        )
...
_BUILTIN_CONCEPTS = {
    "runbook": (
        ...
        "**Key fields:**\n"
        "- `platform`: List of platform configs "
        "(azure, hyperv, aws, baremetal, ready)\n"
    ...
    "environment": (
        ...
        "(CPU, memory, features, OS) against what each platform can provide."  # noqa: E501
    ),
    "node": (
        "## Node\n\n"
    ...
    "platform": (
        ...
        "infrastructure: `azure`, `hyperv`, `qemu`, `cloud-hypervisor`, "
        "`baremetal`, `aws`, or `ready`.\n\n"
        "Configured in the runbook's `platform:` section with type-specific fields.\n"
        "Each platform implements node creation, lifecycle, and capability "
        "reporting.\n\n"
        "`local` and `remote` are **node** types, not platforms. Pair them with "
        "the `ready` platform, which provisions nothing:\n\n"
        "```yaml\n"
        "environment:\n"
        "  environments:\n"
        "    - nodes:\n"
