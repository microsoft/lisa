...
def register_log_analysis_tools(mcp: FastMCP) -> None:
    @mcp.tool()
    def lisa_analyze_log(
        ...
        if not sections:
            sections.append("No structured test results found in the log. "
    ...
    def lisa_explain_failure(failure_text: str) -> str:
        ...
        if any(k in text_lower for k in [
            "kernel panic", "kernel bug", "call trace", "rip:",
            "bug: soft lockup", "oops", "general protection fault",
        ]):
        ...
        # SSH / connectivity
        if any(k in text_lower for k in [
            "tcpconnectionexception", "ssh", "connection refused",
            "connection timed out", "tcp port", "paramiko",
        ...
        # Provisioning
        if any(k in text_lower for k in [
            "provisioningerror", "deployment failed", "allocation failed",
            "overconstrainedallocationrequest", "operationnotallowed",
            "resourcenotfound", "quotaexceeded",
        ]):
        ...
        # Assertion failures
        if any(k in text_lower for k in [
            "assertionerror", "assert_that", "expected", "to be equal to",
            "to contain", "is not true", "is not false",
        ]):
    ...
    def lisa_summarize_run(
        ...
        lines = []
        lines.append(f"## LISA Run Summary")
        lines.append("")
    ...
    def lisa_download_logs(
        ...
        try:
            ...
            file_count = sum(
                1 for _, _, files in os.walk(result_dir) for _ in files
            )
    ...
    def lisa_list_log_files(
        ...
        **Tip:** Start here to understand what logs are available, then
        ...
        if not found:
            return (
                f"No files matching `{file_extensions}` in `{folder_path}`"
            )
    ...
    if path:
        ...
        if size > _MAX_LOG_SIZE:
            return (
                ...
                f"5 MB limit. Provide a trimmed version or the relevant section."
            )
...
def _find_test_source(repo_root: Path, test_name: str) -> str:
    ...
    for testsuites_dir in testsuites_dirs:
        ...
        for py_file in testsuites_dir.rglob("*.py"):
            ...
            return f"**Source: `{rel_path}` (lines {start+1}–{end})**\n\n```python\n{source_block}\n```"
...
def _generate_debug_guidance(failure_log: str, source: str) -> str:
    ...
    steps.append("1. **Check the full stack trace** — the bottom of the traceback "
    ...
    if "ssh" in log_lower or "tcp" in log_lower:
        steps.append("2. **Check serial console** — the VM may have panicked during boot.")
    ...
    if not any(k in log_lower for k in ["assert", "timeout", "ssh", "tcp"]):
        steps.append(
            ...
            f"-v \"testcase.name:<test_name>\"` to reproduce."
        )
        steps.append(
...
def _download_azure_blob_prefix(
    ...
    """
    DefaultAzureCredential, BlobServiceClient = _get_azure_imports()
    ...
    if not blobs:
        raise FileNotFoundError(
            f"No blobs found under '{container}/{prefix}'."
        )
    ...
    leaf_name = (
        normalized.rsplit("/", maxsplit=1)[-1] if normalized else container
    )
    ...
    for blob in blobs:
        ...
        safe_parts = [
            p for p in relative.split("/") if p and p != "." and p != ".."
        ]
        if not safe_parts:
            continue
