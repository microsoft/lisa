...
def _call(tool_name: str, **kwargs: object) -> str:
    ...
    assert tool_name in tools, f"Tool '{tool_name}' not found. Available: {sorted(tools)}"
...
class TestGetTestWriterGuidelines(unittest.TestCase):
    def test_returns_guidelines(self) -> None:
        ...
        self.assertTrue(
            "gather" in result.lower() or "research" in result.lower()
...
class TestListTestRequirements(unittest.TestCase):
    def test_nonexistent_test(self) -> None:
        result = _call("lisa_list_test_requirements", test_name="nonexistent_test_xyz_123")
