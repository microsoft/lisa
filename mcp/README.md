...
`storage.azure.com`. A hosted server therefore refuses to authenticate to any
account not named in `LISA_ALLOWED_STORAGE_ACCOUNTS` — otherwise a caller
could point it at their own account and capture the credential. SAS URLs are
unaffected: they carry their own token and need no allowlist entry.
