    ...
    try {
        foreach ($TestCaseData in $TestCaseList) {
            ...
            if ($reader.Read()) {
                ...
                if ($testCategory -ne $TestCaseData.Category -or $testArea -ne $TestCaseData.Area -or $testPriority -ne $TestCaseData.Priority -or
                    $testTags -ne $TestCaseData.Tags -or $testPlatform -ne $TestCaseData.Platform) {
                    $sqlQuery = "Update $tableName Set TestCategory='$($TestCaseData.Category)', TestArea='$($TestCaseData.Area)', TestPriority='$($TestCaseData.Priority)',
