...
function Sync-Files {
    ...
    if (-not $ckwinscp) {
        Write-Host "Installing WinSCP Powershell Wrapper..."
    ...
    if (-not $move) {
        Write-Error "Failed to perform FTP operation..."
    }
    # Remove the WinSCP session after completion.
...
Function Main {
    ...
    # expected folder naming is YEAR-MM-DD, example 2019-03-09
    ...
    Sync-Files -Username $FTPUser -Password $FTPPass -Hostname $FTPHost -SrcPath "${CURRENT_DIR}\${SrcDest}" `
        -DesPath "/site/wwwroot/pages/$SrcDest"
    ...
    Sync-Files -Username $FTPUser -Password $FTPPass -Hostname $FTPHost -SrcPath "${CURRENT_DIR}\js\pageData.js" `
        -DesPath "/site/wwwroot/js/pageData.js"
