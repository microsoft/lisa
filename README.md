...
3. There are two ways to run LISAv2 tests:
  * Note when testing with 'Azure' platform:
    ...
    * '-ARMImageName' is optional when testing with Azure Gallery Images ('-OsVHD' is not provided to Run-LISAv2.ps1 parameters). If '-ARMImageName' and '-OsVHD' are not provided from Run-LISAv2.ps1 parameters, LISAv2 will try to use <DefaultARMImageName> from .\XML\GlobalConfigurations.xml as value of '-ARMImageName'. In this case, if <DefaultARMImageName> is not defined from .\XML\GlobalConfigurations.xml, exeption will be thrown.

    * '-TestLocation' is optional. If '-TestLocation' is not provided from Run-LISAv2.ps1 parameters, LISAv2 will use the pre-defined <TestLocation> from test definition xml for each test case. If test case does not pre-define any <TestLocation>, LISAv2 will choose an enabled Azure Region automatically per test case Vm Size and Vm Family from current subscription context. The auto-selected TestLocation/Region is likely to have the most available Compute vCPUs resources for the target Vm Size and Vm Family of current test case. The available vCPU resources of all enabled Azure Regions is calculated and ordered by LISAv2 dynamically based on the current Compute vCPUs resource usage of current subscription when necessary. Without '-TestLocation' from Run-LISAv2.ps1 can be used when vCPU resources between Azure Regions are not ideal for current testing pass and selected test cases. If LISAv2 could not deploy from any enabled regions for a target Vm Size, it will wait for at most 1 hour for any released Compute resources, and then continue test deployment when resource available, or abort the current test case when waiting timeout.

    * Selected test cases which is selected by '-TestCategory', '-TestPriority', '-TestArea', '-TestTag', can be expanded and tested with flexible setup parameters, such as '-ARMImageName "<publisher  offer  SKU  version>(,<publisher  offer  SKU  version>)..."', '-OverrideVMSize "<Azure_Vm_Size>(,<Azure_Vm_Size>)..."', '-TestLocation "<Azure_Region>(,<Azure_Region>)..."'. These setup parameters may contain multiple values, separated by ',' which means the selected test cases can be tested multiple times with different ARMImageNames, or different Vm Sizes, or different Azure Regions/Locations. If Comma-Separated values used in '-ARMImageName' '-OverrideVMSize', '-TestLocation', the overall test case execution times will be: (Count of Test Cases) * (Count of ARMImageNames) * (Count of TestLocations/Regions) * (Count of OverrideVMSizes) * (Number of TestIterations)

    * If a Test Case has designed a pre-defined value for '<ARMImanageName>', <OverrideVMSize>, <TestLocation> from its definition xml, then the pre-defined setup configurations of this Test Case will be respected and used for test case environment deployment on Azure before any of test case scripts eventually being invoked, unless there is a '-ForceCustom' parameter applied to Run-LISAv2.ps1. In another words, the '-ForceCustom' will choose the test setup parameters like '-ArmImageName', '-TestLocation', '-OverrideVMSize' from Run-LISAv2 command line and force override the test case pre-defined setup configurations for its test environment.

    * The test parameters of '-ARMImageName', '-TestLocation', '-OverrideVMSize' are semantically equivalent to be used as one of CustomParameters, just like 'Network=xxx;OSType=yyy;DiskType=mmm;ImageType=nnn;'. For example: `-ARMImageName "Redhat RHEL 7.7 Latest,Canonical UbuntuServer 18.04-LTS Latest"` are semantically equivalent to `-CustomParameters "ARMImageName=Redhat RHEL 7.7 Latest,Canonical UbuntuServer 18.04-LTS Latest"`
