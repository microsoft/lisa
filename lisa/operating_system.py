...
class OperatingSystem:
    ...
    def _get_detect_string(cls, node: Any) -> Iterable[str]:
        ...
        cmd_result = typed_node.execute(
            cmd="lsb_release -d", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__lsb_release_pattern
        )

        cmd_result = typed_node.execute(
            cmd="cat /etc/os-release", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__os_release_pattern_name
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__os_release_pattern_id
        )
        cmd_result_os_release = cmd_result

        # for RedHat, CentOS 6.x
        cmd_result = typed_node.execute(
            cmd="cat /etc/redhat-release", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__redhat_release_pattern_header
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__redhat_release_pattern_bracket
        )

        # for FreeBSD
        cmd_result = typed_node.execute(cmd="uname", no_error_log=True, timeout=30)
        yield cmd_result.stdout

        # for Debian
        cmd_result = typed_node.execute(
            cmd="cat /etc/issue", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__debian_issue_pattern
        )

        # note, cat /etc/*release doesn't work in some images, so try them one by one
        # try best for other distros, like Sapphire
        cmd_result = typed_node.execute(
            cmd="cat /etc/release", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(cmd_result, cls.__release_pattern)

        # try best for other distros, like VeloCloud
        cmd_result = typed_node.execute(
            cmd="cat /etc/lsb-release", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(cmd_result, cls.__release_pattern)

        # try best for some suse derives, like netiq
        cmd_result = typed_node.execute(
            cmd="cat /etc/SuSE-release", no_error_log=True, timeout=30
        )
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__suse_release_pattern
        )

        cmd_result = typed_node.execute(cmd="wcscli", no_error_log=True, timeout=30)
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__bmc_release_pattern
        )

        cmd_result = typed_node.execute(cmd="vmware -lv", no_error_log=True, timeout=30)
        yield cls._get_matched_str_from_cmd_result(
            cmd_result, cls.__vmware_esxi_release_pattern
        )
    ...
    @classmethod
    def _get_matched_str_from_cmd_result(
        cls, cmd_result: ExecutableResult, pattern: Pattern[str]
    ) -> str:
