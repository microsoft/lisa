...
class StoragePerformance(TestSuite):
    ...
    @TestCaseMetadata(
        description=(
            "This test case uses fio to test premium data disk performance with 4K "
            "block size using libaio, with one worker per disk pinned to a logical "
            "CPU.\n"
        ...
        requirement=simple_requirement(
            disk=schema.DiskOptionSettings(
                ...
                data_disk_size=search_space.IntRange(min=8192),
                data_disk_count=search_space.IntRange(min=16),
            ),
        ),
        tags=["ai-generated"],
    )
    def perf_premium_datadisks_4k_cpu_affinity(
        self, node: Node, result: TestResult
    ) -> None:
        perf_premium_datadisks(node, result, cpu_affinity=True)
