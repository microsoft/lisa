...
class XfstestsParallelRunner:
    ...
    def run_parallel(
        ...
        # Track the worker path for each queued task so Phase 2 can map results
        # back to the right worker directory. Empty batches are skipped here, so
        # a positional index into worker_paths would misalign; keep the paths
        # alongside the tasks and zip them with the results instead.
        queued_paths: List[PurePath] = []
        for worker_id, batch in enumerate(test_batches, start=1):
            if batch:
                ...
                queued_paths.append(worker_path)
                self.log.debug(
        ...
        # run_in_parallel preserves input order, so queued_paths[i] matches
        # execution_results[i]; zip them so each result maps to its own worker
        # directory even when some batches were empty.
        final_results: List["XfstestsRunResult"] = []
        for worker_path, exec_result in zip(queued_paths, execution_results):
            if not exec_result.success:
                # Execution error occurred - skip result collection.
                # The failure is escalated later by aggregate_results(), so log
                # at info here rather than warning.
                self.log.info(
            ...
            # Log names are derived from the run_id; worker_path comes from the
            # queued_paths list paired with this result above.
