...
from lisa import LisaException, SkippedException, simple_requirement
...
class SanitySuite(TestSuite):
    ...
    def verify_vm_not_compromised(self, node: Node) -> None:
        """
        ...
        node.log.info("Running non-marketplace VM comprehensive sanity checks...")
        try:
            self._check_basic_integrity(node)
            node.log.info("All sanity checks completed successfully.")
        except Exception as integrity_check_error:
            raise LisaException(f"Sanity checks failed: {integrity_check_error}")

    def _get_imds_compute_url(self, api_version: str = "2021-02-01") -> str:
    ...
    def _is_marketplace_image(self, node: Node) -> bool:
        ...
        """
        imds_url = self._get_imds_compute_url(self.IMDS_API_VERSION)
        ...
        for attempt_number in range(self.IMDS_MAX_RETRIES + 1):
            try:
                ...
                node.log.debug(f"Detected {image_type} image")
        ...
        node.log.debug("All IMDS attempts failed, defaulting to non-marketplace image")
        return False

    def _check_ssh_security(self, node: Node) -> None:
        """
            ...
            if existence_response.exit_code == 0:
                ...
                if (
                    ...
                    try:
                        ...
                        node.log.debug(
                            f"Could not parse mode for {file_path}: {parsing_error}"
                        )

    def _check_user_accounts(self, node: Node) -> None:
        """
        ...
        if uid_check_response.exit_code == 0 and uid_check_response.stdout.strip():
            ...
            node.log.debug(
                f"User account UID check passed. UID 0 users: {uid_zero_users}"
            )
        else:
            node.log.debug(
    ...
    def _check_basic_integrity(self, node: Node) -> None:
        """
        ...
        """
        node.log.debug("Starting basic integrity checks...")
        ...
        node.log.debug("Critical system files existence check passed")
        ...
        node.log.debug("Shadow file permissions check passed")
        ...
        node.log.debug("SSH config permissions check passed")

        # 4) SSH security checks (new)
        self._check_ssh_security(node)
        node.log.debug("SSH security checks passed")

        # 5) User account checks (new)
        self._check_user_accounts(node)
        node.log.debug("User account checks passed")
