...
class KexecSuite(TestSuite):
    ...
    def _run_kexec_test(
        ...
        # Create marker in memory (contains nonce for cmdline validation)
        marker = self._create_marker(kernel_path, initrd_path)
        log.info(f"Created marker with nonce: {marker['nonce']}")

        # Load kexec image with cmdline marker
        self._load_kexec_image(
            node, kernel_path, initrd_path, before_state["cmdline"], marker, log
        )
    ...
    def _create_marker(
        ...
        """Create in-memory marker with kexec-specific tracking data."""
        return {
    ...
    def _load_kexec_image(
        ...
        Appends a unique cmdline marker to prove kexec reboot occurred.
        """
        ...
        # Generate unique cmdline marker using the marker nonce
        # This proves we actually booted via kexec (not firmware reboot)
        kexec_marker = f"lisa_kexec_nonce={marker['nonce']}"
