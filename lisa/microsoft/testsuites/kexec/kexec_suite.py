...
class KexecSuite(TestSuite):
    ...
    def verify_kexec_reboot(self, node: Node, log: Logger) -> None:
        ...
        # Check kexec support
        self._check_kexec_support(node, log)
        ...
        self._write_marker(node, marker, log)
        ...
        # Validate the reboot
        self._validate_kexec_reboot(node, marker, before_state, after_state, log)

        # Cleanup marker
        self._cleanup_marker(node, log)

        log.info("Kexec reboot test completed successfully")

    def _check_kexec_support(self, node: RemoteNode, log: Logger) -> None:
        """
        Check if kernel has kexec support enabled.

        Raises SkippedException if kexec is not supported.
        """
        log.debug("Checking kexec kernel support...")
        ...
        if (
            "CONFIG_KEXEC=y" in config_result.stdout
            or "CONFIG_KEXEC_FILE=y" in config_result.stdout
        ):
            log.debug("Kernel config confirms kexec support")
            return

        # Fallback: check for kexec sysfs or command availability
        if node.execute("test -d /sys/kernel/kexec_loaded", sudo=True).exit_code == 0:
            log.debug("kexec sysfs interface found")
            return
        ...
        if kexec_check.exit_code != 0:
            ...
            if verify_check.exit_code != 0:
                raise SkippedException(
                    ...
                    "Kernel may not support kexec."
                )
            log.info("kexec-tools installed successfully")

        # If we got here, kexec tool is available
        log.debug("kexec tool is available, proceeding with test")
    ...
    def _record_before_state(self, node: RemoteNode, log: Logger) -> Dict[str, Any]:
        ...
        boot_id = node.execute(
            "cat /proc/sys/kernel/random/boot_id", sudo=True
        ).stdout.strip()
    ...
    def _write_marker(
        ...
        node.execute(write_cmd, sudo=True, shell=True)
    ...
    def _load_kexec_image(
        ...
        # Build kexec load command
        kexec_cmd = (
            f"kexec -l {kernel_path} "
        ...
        # Verify load status
        verify_result = node.execute("cat /sys/kernel/kexec_loaded", sudo=True)
        if "1" not in verify_result.stdout:
            log.warning("kexec_loaded sysfs shows unexpected value")
    ...
    def _trigger_kexec_reboot(self, node: RemoteNode, log: Logger) -> None:
        ...
        if has_systemctl:
            ...
            try:
                # This will disconnect, so don't expect a response
                node.execute(
                    "systemctl kexec",
                    sudo=True,
                    timeout=5,
    ...
    def _is_system_connected(self, node: RemoteNode, log: Logger) -> bool:
        ...
        """
        try:
            try_connect(node._connection_info, ssh_timeout=10)  # type: ignore
            return True
        except Exception as e:
    ...
    def _validate_kexec_reboot(
        ...
        # Read marker file
        marker_path = PurePosixPath(self.MARKER_DIR) / self.MARKER_FILE
        ...
        # Validate uptime reset (should be much smaller)
        if after_state["uptime"] >= before_state["uptime"]:
            raise AssertionError(
                f"Uptime did not reset! Before: {before_state['uptime']}s, "
                f"After: {after_state['uptime']}s"
            )

        log.info(f"Uptime reset: {before_state['uptime']}s -> {after_state['uptime']}s")
    ...
    def _cleanup_marker(self, node: RemoteNode, log: Logger) -> None:
        """Remove marker file after successful validation."""
        marker_path = PurePosixPath(self.MARKER_DIR) / self.MARKER_FILE
