...
def check_pmd_support(node: Node, pmd: Pmd) -> None:
    ...
    for nic in node.tools[Lspci].get_devices_by_type(DEVICE_TYPE_SRIOV):
        if "Connect-X3" in nic.device_info and not node.tools[KernelConfig].is_enabled(
            "CONFIG_MLX4_INFINIBAND"
        ):
            raise SkippedException(
                "This VM uses Connect-X3 but does not have the mlx4_ib driver."
            )
