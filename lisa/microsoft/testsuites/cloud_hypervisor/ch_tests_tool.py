...
class CloudHypervisorTests(Tool):
    ...
    def _extract_stdout_diagnostics(self, stdout: str) -> List[str]:
        ...
        # Only check for hung tests if we don't already have explicit test failures
        # (failed tests have their own diagnostic messages, hung test detection is for timeouts)
        if not failed_tests:
