...
def get_network_driver_errors(node: Node) -> str:
    """Return dmesg errors for accelerated networking devices."""
    drivers = node.nics.get_accelerated_networking_drivers()
    if not drivers:
        return ""
    ...
    for driver in drivers:
        ...
        for error in (match for matches in errors for match in matches):
            driver_errors.append(
                f"{driver} error found in dmesg: {' '.join(error.split())}"
            )
    return "; ".join(driver_errors)
    ...
    for node in environment.nodes.list():
        ...
        if is_sriov:
            ...
            if pci_nic_count != sriov_count:
                driver_error = get_network_driver_errors(node)
