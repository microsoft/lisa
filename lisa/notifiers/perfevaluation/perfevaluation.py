...
class PerfEvaluation(notifier.Notifier):
    ...
    def _modify_message(self, message: messages.MessageBase) -> None:
        ...
        if isinstance(message, TestResultMessage):
            ...
            # Strategy 1: Try full_name
            if message.full_name in self._failed_metrics:
                test_case_name = message.full_name
                self._log.debug(f"[DEBUG] Matched by full_name: {test_case_name}")
            # Strategy 2: Try name
            elif message.name in self._failed_metrics:
                test_case_name = message.name
                self._log.debug(f"[DEBUG] Matched by name: {test_case_name}")
            # Strategy 3: Try fuzzy match - check if any failed metric key
            # is in the message names
            else:
                for key in self._failed_metrics.keys():
                    if key in message.full_name or key in message.name:
                        test_case_name = key
                        self._log.debug(
                            f"[DEBUG] Matched by fuzzy match: {test_case_name}"
                        )
                        break

            if not test_case_name:
                self._log.debug(
                    f"[DEBUG] No failed metrics found for test: "
                    f"{message.name} / {message.full_name}"
                )
                return
