...
class SerialConsole(Feature):
    ...
    def check_panic(
        ...
        if panics:
            ...
            # Store relative path for reference (useful when logs are uploaded)
            # Format: <node_name>/<datetime>/serial_console.log
            relative_log_path = (
                f"{self._node.name}/{get_datetime_path()}/{NAME_SERIAL_CONSOLE_LOG}"
            )
            ...
            panic_info = PanicInfo(
                ...
                console_log_path=relative_log_path,
            )
