...
class PanicInfo:
    ...
    console_log_path: Optional[Path]  # Path to serial_console.log file within LISA logs
...
class SerialConsole(Feature):
    ...
    def check_panic(
        ...
        Raises:
            KernelPanicException for backward compatibility if stage is provided.
        """
        ...
        if panics:
            ...
            # Determine console log path (get_console_log handles path creation)
            console_log_path = None
            if saved_path:
                console_log_path = (
                    saved_path / get_datetime_path() / NAME_SERIAL_CONSOLE_LOG
                )
            else:
                # Use node's local log path
                console_log_path = (
                    self._node.local_log_path
                    / get_datetime_path()
            ...
            panic_info = PanicInfo(
                ...
                console_log_path=(
                    console_log_path if console_log_path.exists() else None
                ),
            )
    ...
    def attach_panic_to_test_result(
        ...
        test_result.information["panic"] = {
            ...
            "console_log_path": (
                str(panic_info.console_log_path)
                if panic_info.console_log_path
                else None
            ),
