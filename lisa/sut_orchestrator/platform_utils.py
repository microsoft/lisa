...
def get_vmm_version(node: Node) -> str:
    """
    Detects cloud-hypervisor VMM version from cached value, local binary, or git repository.
    ...
    try:
        ...
        if hasattr(node.capability, "extended_resources") and node.capability.extended_resources:
            cached_version = node.capability.extended_resources.get("vmm_version", "UNKNOWN")
        ...
        local_result = node.execute("cloud-hypervisor --version", shell=True, sudo=False)
        ...
        # Use standard LISA tool path where CloudHypervisorTests clones the repo
        git_repo_path = "~/lisa_working/tool/cloudhypervisortests/cloud-hypervisor"
        git_cmd = f"bash -c 'cd {git_repo_path} 2>/dev/null && git describe --tags --always --dirty 2>&1'"
