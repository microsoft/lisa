...
class AzurePlatform(Platform):
    ...
    def _create_node_arm_parameters(
        ...
        if (
            hyperv_generation
            and isinstance(hyperv_generation, features.HyperVGenerationSettings)
            and isinstance(hyperv_generation.gen, int)
        ):
            arm_parameters.hyperv_generation = hyperv_generation.gen
        else:
            raise LisaException("Could not determine Hyper-V Generation.")
