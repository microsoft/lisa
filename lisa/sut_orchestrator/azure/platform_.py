...
class AzurePlatform(Platform):
    ...
    def _get_normalized_vm_sizes(
        ...
        matched_vm_sizes = list(dict.fromkeys([x for x in matched_vm_sizes if x]))
        log.debug(f"matched VM sizes: {matched_vm_sizes}")
        return matched_vm_sizes

    def _get_vm_sizes_by_family(
        self, family_name: str, location: str, log: Logger
    ) -> List[str]:
        length_check_input = family_name.strip()
        # "Family" is an optional suffix in family name, remove it before length check
        # to be more flexible on input.
        length_check_input = re.sub(
            "family", "", length_check_input, flags=re.IGNORECASE
        )
        if len(length_check_input) < MIN_VM_FAMILY_TOKEN_LENGTH:
            raise LisaException(
                "VM SKU family name must be at least "
                f"{MIN_VM_FAMILY_TOKEN_LENGTH} characters: "
                f"'{family_name}'. Example: standardDSv2Family or standardDSv2"
            )
