...
class AzurePlatform(Platform):
    ...
    def _create_node_runbook(
        ...
        if azure_node_runbook.vhd and azure_node_runbook.vhd.vhd_path:
            ...
            azure_node_runbook.hyperv_generation = _get_gallery_image_generation(
                self._get_detailed_cgi(azure_node_runbook.community_gallery_image)
            )
    ...
    def _get_cgi_info(
        ...
        compute_client = get_compute_client(self)
        cgi_info = compute_client.community_gallery_image_versions.get(
            location=community_gallery_image.location,
            public_gallery_name=community_gallery_image.image_gallery,
            gallery_image_name=community_gallery_image.image_definition,
        ...
        assert isinstance(
            cgi_info, CommunityGalleryImageVersion
        ), f"actual: {type(cgi_info)}"
        return cgi_info

    @lru_cache(maxsize=10)  # noqa: B019
    def _get_detailed_cgi(
        self, community_gallery_image: CommunityGalleryImageSchema
    ) -> CommunityGalleryImage:
        compute_client = get_compute_client(self)
        detailed_cgi = compute_client.community_gallery_images.get(
            location=community_gallery_image.location,
            public_gallery_name=community_gallery_image.image_gallery,
            gallery_image_name=community_gallery_image.image_definition,
        )
        assert isinstance(
            detailed_cgi, CommunityGalleryImage
        ), f"actual: {type(detailed_cgi)}"
        return detailed_cgi
    ...
    def _get_cgi_os_disk_size(
        ...
        found_image = self._get_cgi_info(community_gallery_image)
    ...
    def _add_image_features(self, node_space: schema.NodeSpace) -> None:
        ...
        if azure_runbook.marketplace:
            ...
            cgi = self._get_detailed_cgi(azure_runbook.community_gallery_image)
