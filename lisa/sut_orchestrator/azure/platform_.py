...
class AzurePlatform(Platform):
    ...
    def _get_allowed_capabilities(
        ...
        if node_runbook.eligible_vm_sizes:
            patterns = re.compile("|".join(node_runbook.eligible_vm_sizes))
            allowed_vm_sizes = [x for x in allowed_vm_sizes_raw if patterns.search(x)]
            if not allowed_vm_sizes:
                error = (
                    f"no vm size found in '{location}', "
                    f"after intersect '{node_runbook.eligible_vm_sizes}'"
                )
                return [], error
