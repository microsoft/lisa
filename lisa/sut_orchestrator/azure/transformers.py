...
@dataclass_json
@dataclass
class DeleteSigTransformerSchema(schema.Transformer):
    # Resource group that owns the gallery resources created by azure_sig.
    gallery_resource_group_name: str = field(
        default="", metadata=field_metadata(required=True)
    )
    # Gallery name containing the image definition.
    gallery_name: str = field(default="", metadata=field_metadata(required=True))

    # Image definition name.
    gallery_image_name: str = field(default="", metadata=field_metadata(required=True))
    # Full image name in the format: "publisher offer sku version".
    # Version will be parsed from the last token.
    gallery_image_fullname: str = field(
        default="", metadata=field_metadata(required=True)
    )
    # If true, cleanup failures are logged and ignored.
    # If false, any cleanup failure raises exception.
    ignore_failure: bool = True


class DeleteSharedGalleryImageTransformer(Transformer):
    """
    Clean up resources generated by azure_sig.

    Behavior:
    1. Parse version from gallery_image_fullname
    2. Delete SIG image version
    3. Delete image definition only if no version remains

    Scope limitation by design:
    - Only deletes image version and image definition.
    - Does NOT delete gallery.
    - Does NOT delete resource group.

    This avoids cross-run/process contention on shared gallery/RG resources.
    """

    @classmethod
    def type_name(cls) -> str:
        return "azure_sig_delete"

    @classmethod
    def type_schema(cls) -> Type[schema.TypedSchema]:
        return DeleteSigTransformerSchema
    ...
    def _internal_run(self) -> Dict[str, Any]:
        runbook: DeleteSigTransformerSchema = self.runbook

        try:
            platform = _load_platform(self._runbook_builder, self.type_name())
            gallery_image_version = self._parse_version_from_fullname(
                runbook.gallery_image_fullname
            )

            self._delete_sig_image_version(
                platform,
                runbook.gallery_resource_group_name,
                runbook.gallery_name,
                runbook.gallery_image_name,
            ...
            self._delete_image_definition_if_empty(
                platform,
                runbook.gallery_resource_group_name,
                runbook.gallery_name,
                runbook.gallery_image_name,
            ...
            if runbook.ignore_failure:
                self._log.exception(
                    "unexpected exception in DeleteSharedGalleryImageTransformer, "
                    f"skip cleanup. {e}"
                )
            else:
                raise
    ...
    def _parse_version_from_fullname(self, gallery_image_fullname: str) -> str:
        parts = gallery_image_fullname.split()
        if len(parts) != 4:
            raise LisaException(
                "gallery_image_fullname should be in format: "
                f"'publisher offer sku version', got: '{gallery_image_fullname}'"
            )
        return parts[3]

    def _delete_sig_image_version(
        self,
        platform: AzurePlatform,
        ...
        compute_client = get_compute_client(platform)
        try:
            operation = compute_client.gallery_image_versions.begin_delete(
                resource_group_name=gallery_resource_group_name,
                gallery_name=gallery_name,
                gallery_image_name=gallery_image_name,
                gallery_image_version_name=gallery_image_version,
            )
            # Wait to avoid stale list result before checking definition cleanup.
            wait_operation(operation, failure_identity="delete gallery image version")
            self._log.info(
                f"deleted sig image version: {gallery_name}/"
                f"{gallery_image_name}/{gallery_image_version}"
            )
        except Exception as e:
            if "ResourceNotFound" in str(e) or "NotFound" in str(e):
                self._log.info(
                    f"sig image version not found: {gallery_name}/"
                    f"{gallery_image_name}/{gallery_image_version}, skip delete."
                )
            else:
                raise

    def _delete_image_definition_if_empty(
        self,
        platform: AzurePlatform,
        gallery_resource_group_name: str,
        gallery_name: str,
        ...
        # Delete definition only when there are no remaining versions.
        # Intentionally do not delete gallery or resource group to reduce
        # concurrent cleanup races across different runs/processes.
        compute_client = get_compute_client(platform)
        try:
            versions = list(
            ...
            if versions:
                self._log.info(
                    f"skip deleting definition {gallery_name}/{gallery_image_name}, "
                    f"remaining version count: {len(versions)}"
                )
                return

            operation = compute_client.gallery_images.begin_delete(
                resource_group_name=gallery_resource_group_name,
                gallery_name=gallery_name,
                gallery_image_name=gallery_image_name,
            )
            wait_operation(operation, failure_identity="delete gallery image")
            self._log.info(
                f"deleted empty image definition: {gallery_name}/{gallery_image_name}"
            )
        except Exception as e:
            if "ResourceNotFound" in str(e) or "NotFound" in str(e):
                self._log.info(
                    f"sig image definition not found: {gallery_name}/"
                    f"{gallery_image_name}, skip delete."
                )
            else:
                raise
