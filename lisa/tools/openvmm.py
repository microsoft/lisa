...
class OpenVmm(Tool):
    ...
    def _build_launch_shell_command(
        ...
        # OpenVMM's management loop expects a tty for its stdio thread. When the
        # process is launched under nohup with stdin redirected from /dev/null,
        # newer builds panic while enabling raw mode. Even with a pseudo-terminal,
        # the control loop will stop the VM if stdin immediately hits EOF. Feed
        # an always-open empty stream into script so readline blocks as it does in
