...
ACCELERATED_NETWORKING_DEVICE_MODULES: Dict[str, str] = {
...
class Lspci(Tool):
    ...
    def get_accelerated_networking_device_module(self, device: PciDevice) -> str:
        pci_id = f"{device.vendor_id}:{device.device_id}".lower()
        return ACCELERATED_NETWORKING_DEVICE_MODULES.get(pci_id, "")
