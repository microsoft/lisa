...
class NvidiaCudaDriver(GpuDriver):
    ...
    def _install_cuda_suse(self) -> None:
        ...
        # Download and install the CUDA repository configuration
        repo_file_url = (
            f"{self.NVIDIA_CUDA_REPO_BASE_URL}/"
            f"sles{major_version}/x86_64/cuda-sles{major_version}.repo"
        )

        self.node.tools[Wget].get(
            repo_file_url,
            "/etc/zypp/repos.d",
            f"cuda-sles{major_version}.repo",
            sudo=True,
        )

        # Refresh only the CUDA repository to avoid authentication prompts
        # from other repos. The repository name is defined in the .repo file.
        repo_name = f"cuda-sles{major_version}-x86_64"
        self.node.execute(
            f"zypper --gpg-auto-import-keys refresh {repo_name}",
            sudo=True,
            timeout=300,
        )

        # Install CUDA packages for SUSE
        # cuda-drivers-550 is specifically selected due to
        # dependency errors with the latest version.
        packages = ["cuda-drivers-550"]
        self.node.os.install_packages(packages, signed=False)
        self._log.info(f"Successfully installed CUDA driver packages: {packages}")
