...
class Reboot(Tool):
    ...
    def _refresh_baremetal_address(self) -> bool:
        platform = getattr(self.node.features, "_platform", None)
        refresh_node_address = getattr(platform, "refresh_node_address", None)
        if not refresh_node_address:
            return False
        try:
            return bool(refresh_node_address(self.node))
        except Exception as e:
            self._log.debug(f"ignorable baremetal IP refresh exception: {e}")
            return False
    ...
    def _refresh_address_from_serial(self, time_out: int) -> bool:
        ...
        address = cast(str, connection[constants.ENVIRONMENTS_NODES_REMOTE_ADDRESS])
                ...
                if marker_start >= 0 and marker_end > marker_start:
                    ...
                    if matched:
                        ...
                        if new_address != address:
                            ...
                            remote_node.update_connection_address(
                                address=new_address,
    ...
    def reboot(self, time_out: int = 300) -> None:
        ...
        if self._is_baremetal_node():
            ...
            address_refreshed = self._refresh_address_from_serial(
                min(180, baremetal_timeout)
            )
            if not address_refreshed:
                self._refresh_baremetal_address()
