...
class Reboot(Tool):
    ...
    def _refresh_address_from_serial(self, time_out: int) -> bool:
        ...
        port = cast(int, connection[constants.ENVIRONMENTS_NODES_REMOTE_PORT])
        username = cast(str, connection[constants.ENVIRONMENTS_NODES_REMOTE_USERNAME])
        password = cast(
            Optional[str], connection[constants.ENVIRONMENTS_NODES_REMOTE_PASSWORD]
        )
        private_key_file = cast(
            Optional[str],
            connection[constants.ENVIRONMENTS_NODES_REMOTE_PRIVATE_KEY_FILE],
        )
                    ...
                    if matched:
                        ...
                        if new_address != address:
                            ...
                            remote_node.set_connection_info(
                                ...
                                private_key_file=private_key_file or "",
