...
class Fstab(Tool):
    ...
    def remove_entry(self, mount_point: str = "", device: str = "") -> None:
        """
        Remove an entry from /etc/fstab.

        Args:
            mount_point: Mount point to remove (e.g., "/mnt/data")
            device: Device to remove (e.g., "/dev/sda1" or "UUID=xxx")

        Note: At least one of mount_point or device must be specified.
        """
        if not mount_point and not device:
            raise LisaException("Must specify either mount_point or device")

        entries = self.get_entries(force_run=True)
        new_entries: List[FstabEntry] = []
        ...
        for entry in entries:
            ...
            if should_remove:
                self._log.info(f"Removing fstab entry: {entry}")
                removed = True
            else:
                new_entries.append(entry)

        if not removed:
            self._log.debug("No matching entry found to remove")
            return
        ...
        cat = self.node.tools[Cat]
        original_content = cat.run(self.FSTAB_PATH, sudo=True, force_run=True).stdout
        ...
        for line in original_content.splitlines():
            ...
            if not stripped or stripped.startswith("#"):
                ...
                # Stop at first non-comment entry
                break
